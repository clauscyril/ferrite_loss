...
void TD_sim(Mesh *mesh, const std::function<real_t(real_t)> &NI_func, real_t Ts, int num_steps, Ferrite ferrite, bool visualization){
    ...
    {   
        ...
        GridFunction r_grid(fespace);
        r_grid.ProjectCoefficient(r_coeff);
        real_t vol = M_PI *lf_p(r_grid);
...
void TD_sim_by_fluxH(Mesh *mesh, const Array<real_t> fluxH_array, real_t Ts, int num_steps, Ferrite ferrite, bool visualization){
    ...
    GridFunction Enx(fespace);
    GridFunction Eny(fespace);
    ...
    // **** Function multiplied by NI in the linear system
...
void TD_sim_by_fluxH(Mesh *mesh, const std::function<real_t(real_t)> &fluxH_func, real_t Ts, int num_steps, Ferrite ferrite, bool visualization){
    Array<real_t> flux_H_array(num_steps);
    for (int i = 0; i<num_steps; i++) {
        flux_H_array[i] = fluxH_func(Ts * i);
    }
    TD_sim_by_fluxH(mesh, flux_H_array, Ts, num_steps, ferrite, visualization);
}
...
void TD_sim_by_flux(Mesh *mesh, const Array<real_t> flux_array, real_t Ts, int num_steps, Ferrite ferrite, bool visualization){
    ...
    real_t phi_Hn = 0; real_t phi_Hnm1 = 0; real_t phi_n = 0; real_t phi_nm1 = 0;
    Array<real_t> fluxH_array(flux_array.Size());
    for (int i = 0; i<fluxH_array.Size(); i++){
        phi_n = flux_array[i];
        phi_Hn = 1/D1 * (phi_n - D3 * phi_nm1);
        
        fluxH_array[i] = phi_Hn;

        phi_nm1 = phi_n;
        phi_Hnm1 = phi_Hn;
    }

    TD_sim_by_fluxH(mesh, fluxH_array, Ts, num_steps, ferrite, visualization);
}

// Same function but taking a function as the input and not an Array (An array is created using the function)
void TD_sim_by_flux(Mesh *mesh, const std::function<real_t(real_t)> &flux_func, real_t Ts, int num_steps, Ferrite ferrite, bool visualization){
    Array<real_t> flux_array(num_steps);  // Array used for calling the function TD_sim_by_flux above
    for (int i = 0; i<num_steps; i++) {
        flux_array[i] = flux_func(Ts * i);
    }
    TD_sim_by_flux(mesh, flux_array, Ts, num_steps, ferrite, visualization);  // Calling the TD_sim_by_flux function defined above
}
