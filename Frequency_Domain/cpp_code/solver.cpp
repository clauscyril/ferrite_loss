...
// Function for computing the power loss 
void GetPowerLoss(Mesh *mesh, real_t f, real_t Bpeak, real_t &NI, Ferrite ferrite, real_t &P_loss_eddy, real_t &P_loss_mag, std::complex<real_t> &flux, const bool visualization) {
    ...
    // For the permeability, if mu_eq is equal to 0, which mean it is not precised at the declaration,
    // then the first order low pass model is used with the cut-off frequency passed at the declaration.
    // It's been made like this to be able to switch easily between the measured and modeled permeabilitty
    std::complex<real_t> mu_eq = ferrite.mu_eq;
    if (mu_eq == 0.){
        real_t fc_mu = ferrite.fc;
        real_t tau = 1./(2.*M_PI*fc_mu);    // cutoff frequency of µ_eq = µ0µr/(1 + tau*j*w) model
        mu_eq = mu / ((real_t)1. + tau * j * omega);
        
    }
    const std::complex<real_t> j_omega_mu = j*omega*mu_eq;


    // The simulation needs the current NI to compute the losses. 
    // here we can impose either NI or the average peak value of the b-field
    // However, for a certain b_peak, the corresponding NI is not known before the simulation.
    // Fortunately the problem is linear. As a result we can impose an arbitrary value for NI (different from 0),
    // compute the b_field corresponding and rescale the solution using the b_peak imposed as a reference.
    if (NI == 0.)
        NI = 1; // If NI is equal to 0, it means that the imposed value is b_peak. Thus setting an arbitrary value for NI 
    ...
    // If the imposed value was Bpeak, we need to rescale the solution (because of the arbitrary NI)
    if (Bpeak != 0.){
